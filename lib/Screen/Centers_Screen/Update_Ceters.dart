...
class UpdateCenterScreen extends StatefulWidget {
  final CentersModel centerModel;

  UpdateCenterScreen({required this.centerModel});

  @override
  _UpdateCenterScreenState createState() => _UpdateCenterScreenState();
}
...
class _UpdateCenterScreenState extends State<UpdateCenterScreen> {
  ...
  void initState() {
    super.initState();
    _centerNameController.text = widget.centerModel.centerName;
    _centerLocationController.text = widget.centerModel.centerLocation;
    _phoneNumberController.text = widget.centerModel.phoneNumber;
    _imageUrl =
        widget.centerModel.media.isNotEmpty ? widget.centerModel.media : null;
  }
  ...
  Future<void> _updateCenter() async {
    ...
    try {
      String media = widget.centerModel.media;

      if (_imageFile != null) {
        media = await _uploadImage(_imageFile!);
      }

      await UpdateCenters().updateCenters(
        centerName: _centerNameController.text,
        centerLocation: _centerLocationController.text,
        phoneNumber: _phoneNumberController.text,
        media: media,
        id: widget.centerModel.id,
      );
